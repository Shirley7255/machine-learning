...
#     # YOUR CODE HERE
    ...
    N = x_train.shape[0]
    # 构造 N x 2 的矩阵：[x, 1]
    # 另一种常见的构造是 [1, x]，但模板提示 [x, 1]
    Xb = np.stack([x_train, np.ones(N)], axis=1)  # Xb 形状: N x 2

    # 确保 y_train 是 N x 1 矩阵以进行矩阵乘法，尽管对于一维 numpy 向量也常自动工作
    y_mat = y_train.reshape(-1, 1)

    # 2. 计算 Xb^T Xb
    XTX = Xb.T @ Xb  # 2 x N @ N x 2 -> 2 x 2
    ...
    # 4. 计算 Xb^T y
    XTy = Xb.T @ y_mat  # 2 x N @ N x 1 -> 2 x 1

    # 5. 计算最终参数 theta: theta = (Xb^T Xb)^-1 Xb^T y
    theta = XTX_inv @ XTy  # 2 x 2 @ 2 x 1 -> 2 x 1

    # 6. 提取 w 和 b
    # 由于 Xb = [x, 1]，所以 theta = [w, b]^T
    w = theta[0, 0]
    b = theta[1, 0]
    return w, b
